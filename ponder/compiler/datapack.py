...
def compile_datapack(
    ...
    # 生成用于存放函数的文件夹
    function_base_directory = f'data/{ponder_name}/function' if version else f'data/{ponder_name}/functions'
    ...
    for command in commands:
        ...
        if time in functions:
            ...
            continue
        functions[time] = ['# Generated by creepe_ponder 本文件由 creepe_ponder 生成\n', command_string]

    # 生成对应的文件内容

    for time, command_list in functions.items():
        datapack_content[f'{function_base_directory}/{time}.mcfunction'] = '\n'.join(command_list)

    # 生成主函数文件
    command_list = ['# Generated by creepe_ponder 本文件由 creepe_ponder 生成\n']
